...
export class AdminFormComponent {
  ...
  constructor(private fb:FormBuilder,private router:Router,private snackBar: MatSnackBar,private http:HttpClient,private authService:AuthService,private adminService:AdminService) {
  ...
  onPincodeInput() {
    ...
    if (pincode.length === 6) {
      ...
      // Reset city field if pincode is not 6 digits long
  ...
  onsubmit(){
    this.adminService.registerAdmin(this.loginForm.value).subscribe({
      next: (data) => {
        console.log(data);
